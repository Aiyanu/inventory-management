...
const SidebarLink = ({ href, icon: Icon, label, isCollapsed }: SidebarLinkProps) => {
    ...
    return (
        <Link href={href}>
            <div className={`cursor-pointer flex item-center ${isCollapsed ? "justify-center py-4" : "justify-start px-8 py-4"} ${isActive ? "bg-blue-200 text-white" : ""}`}
...
const Sidebar = () => {
    ...
    return <div className={sidebarClassNames}>
        <div className="flex gap-3 justify-between md:justify-normal items-center pt-8">
            <div>logo</div>
            <h1 className="font-extrabold text-2xl">AiStocks</h1>
            <button className="cursor-pointer md:hidden px-3 py-3 bg-gray-100 rounded-full hover:bg-blue-100" onClick={toggleSidebar}><Menu className="w-4 h-4 cursor-pointer" /></button>
        </div>
        ...
        <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
            <p className="text-center text-xs text-gray-500">&copy; 2025 AiStocks</p>
        </div>
    </div>
}
